...
BufferRef Job::trait_buf() const noexcept {
  return buffer_;
}

std::size_t Job::trait_buf_size() const noexcept {
  return buffer_size_;
}
...
JobChannel::channel_r JobChannel::trait_recv() noexcept {
  ...
  if (!w_r) {
    ...
    return jobs_[head_++ % MaxJobs];
  }
}

ChannelError JobChannel::trait_send(const Job& job) noexcept {
  std::unique_lock<std::mutex> lock(mtx_);
  if (trait_full()) {
...
void JobChannel::trait_notify() noexcept {
